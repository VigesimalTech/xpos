...
type PosUser = { name: string; pos_profile: string };

async function posUsersFor(keys: Keys): Promise<Map<string, string>> {
	const res = await fetch(`${site.url}/api/method/xpos.api.auth.get_pos_users?limit_page_length=500`, {
	...
	return new Map(body.message.map((u) => [u.name, u.pos_profile]));
}
